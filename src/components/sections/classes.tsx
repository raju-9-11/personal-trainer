...
// Bolt Optimization: Extract ClassCard and memoize it to prevent unnecessary re-renders
// of the entire list when the parent state (like selectedClass) changes.
const ClassCard = memo(({ item, index, onBook }: { item: GymClass, index: number, onBook: (item: GymClass) => void }) => {
  return (
    <motion.div
...
export function Classes() {
  ...
  // Bolt Optimization: Use useCallback to ensure the handler is stable, allowing React.memo to work on ClassCard
  const handleBook = useCallback((item: GymClass) => {
          ...
          <Carousel className="w-full max-w-5xl mx-auto">
            ...
            {items.map((item, idx) => (
              <CarouselItem key={item.id} className="pl-4 md:basis-1/2 lg:basis-1/3 p-4">
                <ClassCard item={item} index={idx} onBook={handleBook} />
              </CarouselItem>
            ))}
            </CarouselContent>
